...
data_dir = "./Figures/"
figdata = get.figure.data.rep(data_dir)
...
figtheme = theme_minimal() +
  theme(
    ...
    panel.spacing.x = unit(24,"points")
  )
...
ps = dlply(D, "scenarioName", function (D) {
  p = ggplot(D) +
    ...
    labs(x = 'Odds of True Effects',
         title = D$scenarioName[1], color = "Bias") +
    figtheme
  
  pp = p + aes(y = `Positive Predictive Value_Rate`) +
